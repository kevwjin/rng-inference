...
def save_sequences(
    ...
    if output_path is None:
        filename = f"{PREFIX}-rng-len{seq_len}-n{num_seqs}-rep{rep}.npz"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Save sequences as "
             "artifacts/<prefix>-rng-len<length>-n<count>-rep<rep>.npz",
    )
    parser.add_argument(
        "--output-path",
        type=Path,
    ...
    rep = rep_arg or \
          next_available_rep(args.sequence_length, args.num_sequences)
