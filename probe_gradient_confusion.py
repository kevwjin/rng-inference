"""Compute gradient-norm confusion matrix and plot it like the loss heatmap."""
...
from probe_gradients import create_loader, grad_vector, load_model
from train_lstm_classifier import get_device, load_split


def collect_grad_matrix(
    en_split: Path,
    zh_split: Path,
    prng_split: Path,
    en_model: Path,
...
def plot_grad_confusion(df: pd.DataFrame, out_png: Path) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Plot gradient-norm confusion matrix.")
    ...
    parser.add_argument(
        "--out-png",
        type=Path,
        default=Path("artifacts/gradient-confusion.png"),
    ...
    plot_grad_confusion(df, args.out_png)
    return 0
